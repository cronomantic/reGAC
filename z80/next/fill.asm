...
;
; Filling an area on a Spectrum Next, which is the Spectrum's fill.
;
; The walk is the same, the pattern is the same and the rule is the same.  What
; it walks is the mask rather than the picture -- draw.asm says why -- and the
; mask is laid out exactly as a Spectrum's screen would be for these rows, row
; by row and thirty two bytes to a row, so every step of it is that machine's
; own code unchanged.
;
; What is this machine's is the colour.  A run of a fill lays down two of them,
; the ink where the pattern is lit and the paper where it is not, a byte to
; every pixel; there is no cell here for them to share.
...
; The address of column zero of row E in the mask, in HL.  The thirty two
; bytes of the row follow in L, so nothing after this needs the address
; working out again.
; Corrupts: AF
row_address:
                ld      a, e
...
; Give the run the colours in force: the ink where the pattern is lit and the
; paper everywhere else, a byte to every pixel.
;
; The pattern is the same eight bits over and over, so which colour a pixel
; gets comes round every eight; and the run is inside one row, which is one
; page of layer 2, so the address counts up in L and never leaves it.
; Corrupts: everything
colour_span:
                ld      a, (fill_row)
                ld      e, a
                ld      a, (fill_left)
                ld      d, a
                call    colour_address          ; and its half of layer 2 shown
                ld      a, (span_pattern)
                ld      c, a
                ld      a, (fill_left)
                and     7
                jr      z, .lined_up
                ld      b, a                    ; start the pattern where the
.turn:                                          ; run does, not where the byte
                rlc     c                       ; does
                djnz    .turn
.lined_up:
                ld      a, (fill_left)
                ld      e, a
                ld      a, (fill_right)
                sub     e                       ; one less than the run's length
                ld      b, a
                inc     b
                ld      de, (line_colour)       ; the ink low, the paper high
.each:
                rlc     c
                ld      a, e                    ; lit: the ink
                jr      c, .lay_it
                ld      a, d                    ; and the paper where it is not
.lay_it:
                ld      (hl), a
                inc     l
                djnz    .each
...
; Wipe the picture and the mask with it: the picture to the paper it starts
; on, and the mask to nothing, because a wiped picture stops no fill.
; Corrupts: everything
gfx_clear:
                ld      hl, MASK
                ld      de, MASK + 1
